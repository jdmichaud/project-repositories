...
class editor ?packing ?show () = object (self)
  ...
  method perrText () =
    prerr_string (text#get_chars 0 text#length); flush stderr

end

let window = GWindow.window ~width:500 ~height:300 ~title:"Poule Graphic Editor" ()
...
let exec_menu = factory#add_submenu "-> Run Poule <-"
...
let launch_graphic_mode =
  ...
  let factory = new GMenu.factory file_menu ~accel_group in
	...
	factory#add_item "Poule" ~key:_E ~callback:editor#perrText;
	
	editor#text#event#connect#button_press
	  ~callback:(fun ev -> let button = GdkEvent.Button.button ev in
		       if button = 3 then begin
			 file_menu#popup 
			   ~button ~time:(GdkEvent.Button.time ev); true
		       end else false);
